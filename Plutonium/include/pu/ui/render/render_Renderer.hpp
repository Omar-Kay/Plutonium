...
struct RendererInitOptions {
    ...
    inline void AddDefaultFontPath(const std::string& font_path) { this->default_font_paths.push_back(font_path); }

    inline void AddExtraDefaultFontSize(const u32 font_size) { this->extra_default_font_sizes.push_back(font_size); }
    ...
    inline void UseRomfs() { this->init_romfs = true; }

    inline void SetInputPlayerCount(const u32 count) { this->pad_player_count = count; }

    inline void AddInputNpadIdType(const u64 type) { this->pad_id_mask |= BITL(type); }

    inline void AddInputNpadStyleTag(const u32 tag) { this->pad_style_tag |= tag; }
};
...
struct TextureRenderOptions {
    ...
    static constexpr TextureRenderOptions Default() { return {NoAlpha, NoWidth, NoHeight, NoRotation}; }
    ...
    static constexpr TextureRenderOptions
    WithCustomAlphaAndDimensions(const u8 alpha, const i32 width, const i32 height) {
...
public:
    ...
    inline bool HasInitialized() { return this->initialized; }

    inline bool HasRomFs() { return this->ok_romfs; }
    ...
    void RenderTexture(
        ...
        const i32 x,
        const i32 y,
        const TextureRenderOptions opts = TextureRenderOptions::Default()
    );
    ...
    inline void RenderRectangleOutline(
        const Color clr,
        const i32 x,
        const i32 y,
        const i32 width,
    ...
    void RenderRoundedRectangleFill(
        const Color clr,
        const i32 x,
        const i32 y,
        const i32 width,
    ...
    void RenderEllipse(const Color clr, const i32 x, const i32 y, const i32 rx, const i32 ry);
    void RenderEllipseFill(const Color clr, const i32 x, const i32 y, const i32 rx, const i32 ry);
    void RenderShadowSimple(
        const i32 x,
        const i32 y,
        const i32 width,
        const i32 height,
    ...
    inline void ResetBaseRenderPosition() { this->SetBaseRenderPosition(0, 0); }

    inline void SetBaseRenderAlpha(const u8 alpha) { this->base_a = alpha; }

    inline void ResetBaseRenderAlpha() { this->base_a = -1; }

    inline void UpdateInput() { padUpdate(&this->input_pad); }

    inline u64 GetButtonsDown() { return padGetButtonsDown(&this->input_pad); }

    inline u64 GetButtonsUp() { return padGetButtonsUp(&this->input_pad); }

    inline u64 GetButtonsHeld() { return padGetButtons(&this->input_pad); }
};
